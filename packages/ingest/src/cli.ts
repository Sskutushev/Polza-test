...
if (command === "companies") await loadCompanies(readArg("--dir", "./data"));
  ...
  await loadReviews(readArg("--file", "./data/review.csv"));
else if (command === "profile-csv")
  await profileCsv(readArg("--file", "./data/review.csv"));
else {
  console.error(
    "Usage: companies --dir ./data | reviews --file ./data/review.csv | profile-csv --file ./data/review.csv",
  );
  process.exit(1);
}
  ...
  try {
    ...
    for (const file of files) {
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        const categoryId = company.category
          ? await upsertLookup(pool, "category", company.category)
          : null;
        const cityId = company.city
          ? await upsertLookup(
              pool,
              "city",
              company.city.name,
    ...
    console.log(
      `companies read=${read} inserted=${inserted} rejected=${rejected}`,
    );
  } catch (error) {
    await pool.query(
      "UPDATE ingest_run SET status='failed', finished_at=now(), rows_read=$1, rows_inserted=$2, rows_rejected=$3 WHERE id=$4",
      [read, inserted, rejected, runId],
    );
    throw error;
  } finally {
...
async function insertRejected(
  ...
  await pool.query(
    ...
    [
      ...
      index,
      payload,
      issue?.code ?? "FIELD_UNKNOWN",
      issue?.message ?? "Unknown parsing issue",
    ],
  );
}
...
async function loadReviews(file: string): Promise<void> {
  ...
  const pool = createPool();
  const run = await pool.query(
    "INSERT INTO ingest_run(source, status, rows_read) VALUES($1, 'running', $2) RETURNING id",
    ["reviews", rows.length],
  );
  const runId = Number(run.rows[0].id);
  let inserted = 0;
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (!company.rows[0]) {
        ...
        await pool.query(
          "INSERT INTO review_quarantine(run_id, payload, code, message) VALUES($1,$2,$3,$4)",
          [runId, row, "ORPHAN_COMPANY_REF", "Cannot link review to company"],
        );
        continue;
      }
      const email = normalizeEmail(row.email);
      const emailCodes =
        email.value?.codes ?? email.issues.map((issue) => issue.code);
      if (email.value?.status === "invalid" || email.issues.length > 0) {
        ...
        await pool.query(
          ...
          [
            runId,
            row,
            emailCodes[0] ?? "EMAIL_SYNTAX_INVALID",
      ...
      await pool.query(
        ...
        [
          cleanText(row.id ?? row.external_id) ??
            `${company.rows[0].id}:${cleanText(row.author) ?? ""}:${cleanText(row.text ?? row.body) ?? ""}`,
          company.rows[0].id,
...
async function profileCsv(file: string): Promise<void> {
  ...
  for (const header of headers) {
    const values = rows
      .map((row) => cleanText(row[header]))
      .filter((value): value is string => Boolean(value));
    lines.push(
      `| ${header} | ${rows.length - values.length} | ${new Set(values).size} | ${values.slice(0, 3).join(", ")} |`,
    );
  }
