...
export function normalizeName(
  raw: unknown,
): Parsed<{ name: string; nameNorm: string }> {
...
export function normalizeCity(
  raw: unknown,
): Parsed<{ name: string; slug: string } | null> {
...
export function normalizeRating(raw: unknown, max = 5): Parsed<number | null> {
  ...
  if (parsed < 0 || parsed > max) {
    return fail({
      ...
      field: "rating",
      rawValue: raw,
      message: `Rating is outside 0..${max}`,
    });
  }
...
export function normalizePhone(raw: unknown): Parsed<NormalizedPhone | null> {
  ...
  if (!/^\+\d{10,15}$/.test(normalized)) {
    return fail({
      ...
      field: "phone",
      rawValue: raw,
      message: "Phone cannot be normalized to E.164",
    });
  }
  return ok({
    e164: normalized,
    raw: value,
    ...(extMatch?.[1] ? { ext: extMatch[1] } : {}),
  });
}

export function normalizeWebsite(
  raw: unknown,
): Parsed<NormalizedWebsite | null> {
  const value = cleanText(raw);
  if (!value) return ok(null);
  try {
    const url = new URL(
    ...
    const isSocial = /(?:vk\.com|facebook\.com|instagram\.com|t\.me)$/.test(
      host,
    );
    ...
    return fail({
      ...
      field: "website",
      rawValue: raw,
      message: "Website cannot be parsed as URL",
    });
  }
}
...
export function normalizeEmail(raw: unknown): Parsed<NormalizedEmail | null> {
  ...
  if (roleLocalParts.has(local.split("+")[0] ?? local))
    codes.push("EMAIL_ROLE_BASED");
  const comparable = `${local.replace(/\+.*/, "")}@${domain}`;
  const status = codes.some((code) => code !== "EMAIL_ROLE_BASED")
    ? "invalid"
    : codes.length > 0
      ? "risky"
      : "valid";
